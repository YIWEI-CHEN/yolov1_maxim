...
def get_logger(logdir, tag='default', log_level=0):
    ...
    if tag is not None:
        fh = logging.FileHandler('%s/%s-%s.txt' % (logdir, tag, get_time_str()))
        fh.setFormatter(logging.Formatter(log_format))
        fh.setLevel(logging.INFO)
        logger.addHandler(fh)
    return Logger(logger, '%s/tf_log/logger-%s-%s' % (logdir, tag, get_time_str()), log_level)
